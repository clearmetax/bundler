...
export async function buyBundle() {
	...
	const txMainSwaps: VersionedTransaction[] = await createWalletSwaps(blockhash, keypairs, lookupTableAccount, bondingCurve, mintKp.publicKey, mintKp.publicKey, program);
...
async function createWalletSwaps(
	...
	for (let chunkIndex = 0; chunkIndex < chunkedKeypairs.length; chunkIndex++) {
		...
		console.log("Txn size:", versionedTx.message.serialize().length);
		if (versionedTx.message.serialize().length > 1232) {
